...
----------- TEST -----------
-- Dig a straight row, until size runs out
function digRow(dir) -- monitored position string, quarry size
    local limit = (data["dir"..dir] > 0) and data["size"..dir] or 1
    while data["pos"..dir] ~= limit do
        -- Returns false if there is bedrock, or the turtle cant move for some reason
...
-- Dig the full layer
data.facing = {"Z","X",data.dirZ} -- Forward declaration
function digLayer()
    load()
    -- Set table containing facing variables
    data.facing = (data.dirX == 0) and {"Z","X",data.dirZ} or {"X","Z",data.dirX}
    
    data.turnDir = true
    local turnF
    while true do
        turnF = data.turnDir and turnRight or turnLeft
        
        -- Dig the row, return false if the row was interrupted by something
        if not digRow(data.facing[1]) then return false end
        
        -- Basically a complex conditional that determines when to end the layer
        if (data.facing[3] > 0 and data.facing[1] == "Z") or (data.facing[3] < 0 and data.facing[1] == "X") then
            if data["pos"..data.facing[2]] >= data["size"..data.facing[2]] then break end
        elseif (data.facing[3] > 0 and data.facing[1] == "X") or (data.facing[3] < 0 and data.facing[1] == "Z") then
            if data["pos"..data.facing[2]] <= 1 then break end
        else
            print("Fatal error: digLayer()")
            error()
        end
